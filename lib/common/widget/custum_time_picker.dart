  ...
  Widget build(BuildContext context) {
    ...
    MyCalendarDatePicker calendarDatePicker() {
      return MyCalendarDatePicker(
        ...
        background: AppColor.home_backround,
...
//todo:this's header
    ...
    return Dialog(
      ...
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)) ?? defaults.shape,
...
class MyCalendarDatePicker extends StatefulWidget {
  ...
  /// change the [key] to create a new instance of the [MyCalendarDatePicker], and
...
//todo:picker
  ...
  Widget build(BuildContext context) {
    ...
    final String month = _localizations.formatMonthYear(_currentDisplayedMonthDate);
    return Stack(
      ...
      children: <Widget>[
        ...
        Padding(
          ...
          child: _DatePickerModeToggleButton(
            mode: _mode,
            title: month.substring(0, month.length - 4).toUpperCase(),
            onTitlePressed: () {
              // Toggle the day/year mode.
              _handleModeChanged(_mode == DatePickerMode.day ? DatePickerMode.year : DatePickerMode.day);
            },
            title2: _localizations.formatYear(_currentDisplayedMonthDate),
          ),
        ),
      ],
    );
                ...
                children: <Widget>[
                  Flexible(
                    child: Text(
                      widget.title,
                      overflow: TextOverflow.ellipsis,
                      style: textTheme.titleMedium?.copyWith(color: AppColor.white),
                    ),
                  ),
                  Flexible(
                    child: Text(
...
/// This is used by the [_MonthPicker] to let its children [_DayPicker]s know
/// what the currently focused date (if any) should be.
class __FocusedDate extends InheritedWidget {
  const __FocusedDate({
    required super.child,
    this.date,
  });
...
class _DayPickerState extends State<_DayPicker> {
  ...
  void didChangeDependencies() {
    super.didChangeDependencies();
    // Check to see if the focused date is in this month, if so focus it.
    final DateTime? focusedDate = __FocusedDate.maybeOf(context);
    if (focusedDate != null && DateUtils.isSameMonth(widget.displayedMonth, focusedDate)) {
      _dayFocusNodes[focusedDate.day - 1].requestFocus();
    }
  }
  ...
  List<Widget> _dayHeaders(TextStyle? headerStyle, MaterialLocalizations localizations) {
    ...
    for (int i = 0; i <= 6; i++) {
      ...
      result.add(ExcludeSemantics(
        child: Center(
            child: Text(weekday,
                style: headerStyle?.copyWith(color: (i == 0 || i == 6) ? AppColor.date : AppColor.white))),
      ));
    }
    return result;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final int daysInMonth = DateUtils.getDaysInMonth(year, month);
    final int dayOffset = DateUtils.firstDayOffset(year, month, localizations);
//todo: this's list of day in month so we have to convert tim size on picker
...
class _DayState extends State<_Day> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color? dayForegroundColor = resolve<Color?>(
        (DatePickerThemeData? theme) => widget.isToday ? theme?.todayForegroundColor : theme?.dayForegroundColor,
        states);
    final Color? dayBackgroundColor = resolve<Color?>(
        (DatePickerThemeData? theme) => widget.isSelectedDay ? theme?.todayBackgroundColor : theme?.dayBackgroundColor,
        states);
    ...
    final BoxDecoration decoration = BoxDecoration(color: dayBackgroundColor, borderRadius: BorderRadius.circular(10));
...
class _YearPickerState extends State<YearPicker> {
  ...
  void initState() {
    super.initState();
    _scrollController =
        ScrollController(initialScrollOffset: _scrollOffsetForYear(widget.selectedDate ?? widget.firstDate));
  }
